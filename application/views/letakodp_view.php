    ...
    <div>
        ...
        <script>
            ...
            <?php if ($this->session->userdata('level') == 'Unit') { ?>
                <?php foreach ($odp as $od) { ?>
                    L.marker([<?= $od->latitude ?>, <?= $od->longitude ?>], {
                        icon: L.icon({
                        ...
                        Swal.fire({
                            imageUrl: '<?= base_url('assets_style/file/' .  $od->foto) ?>',
                            title: '<span class="text-uppercase">Nama Unit : <?= $od->nama ?></span>',
                            html: "Alamat : <?= $od->alamat ?><br> Luas Area : <?= $od->luas ?> ha",
                            imageHeight: 180,
                            ...
                            if (result.isConfirmed) {
                                window.location = '<?= base_url('odp/detail/' . $od->id_odp) ?>';
                            }
                        })
                    });
                <?php } ?>
                ...
                <?php foreach ($unit_admin as $isi) { ?>
                    L.marker([<?= $isi->latitude ?>, <?= $isi->longitude ?>], {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location = '<?= base_url('odp/detail/' . $isi->id_unit) ?>';
                            }
                        })
                    });
                <?php } ?>
